 ...
 * @LastEditors: Huccct
 * @LastEditTime: 2023-02-12 18:38:30
-->
<template>
  <div class="tas-datepicker" v-click-outside>
    <tass-input prefix-icon="calendarfull" placeholder="请选择日期" :value="formatDate(time)" />
...
<script setup lang="ts">
  ...
  let tYear: string | number = '';
  let tMonth: string | number = '';
  ...
  const isNotCurrentMonth = (date: Date) => {
    ...
    let { year: y, month: m } = utils.formatTime(props.time);
  ...
  const getVisibleDays = computed(() => {
    let { year, month } = utils.formatTime(props.time);
