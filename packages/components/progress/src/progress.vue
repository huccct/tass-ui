<template>
  <div class="tas-progress" v-if="props.select == 'on'">
    ...
    <div class="percent" v-else-if="percent == 100">满</div>
    <div class="percent" v-else-if="props.type != 'in'">{{ props.percent }}%</div>
  </div>

  <div class="tas-progress-circle" :style="{ width, height }" v-else>
    <svg
      viewBox="0 0 96 96"
      class="tas-progress-circle__svgCircle"
    ...
    <div class="tas-progress-circle__mask" v-if="status">
      <tass-icon name="warning" v-if="status == 'warning'"></tass-icon>
      <tass-icon name="success" v-if="status == 'success'"></tass-icon>
    </div>
    <div class="tas-progress-circle__mask" v-else-if="props.type != 'in'">{{ props.percent }}%</div>
  </div>
</template>
